...
class AboutPageTests(unittest.TestCase):
    ...
    def test_commit_about_2(self):
        with app.app_context():
            about_data = about()
            self.assertEqual(about_data.json()['about'][0]['name'], 'Yulissa Montes')

    def test_commit_about_3(self):
        with app.app_context():
            about_data = about()
            self.assertTrue(about_data.json()['about'][0]['stats']['commits']>0)

    def test_commit_about_4(self):
        with app.app_context():
            about_data = about()
            self.assertTrue(about_data.json()['about'][0]['stats']['issues']>0)
